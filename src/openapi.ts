    ...
    schemas: {
      ...
      CheckStatusResponse: {
        oneOf: [
          ...
          { $ref: '#/components/schemas/ExtractReport' },
      ...
      ExtractReport: {
        ...
        properties: {
          ...
          sourceUrl: {
            type: 'string',
            format: 'uri',
            description: 'Final URL with query values redacted.',
          },
          markdown: { type: 'string', maxLength: 48050 },
          contentHash: { type: 'string', pattern: '^[a-f0-9]{64}$' },
          inputBytes: { type: 'integer', maximum: 1048576 },
          outputBytes: { type: 'integer' },
          estimatedInputTokens: { type: 'integer' },
          estimatedOutputTokens: { type: 'integer', maximum: 12020 },
          warnings: { type: 'array', items: { type: 'string' } },
          provenance: {
            type: 'object',
            required: ['fetchedAt', 'redirects', 'contentType', 'parser'],
            properties: {
              fetchedAt: { type: 'string', format: 'date-time' },
              redirects: { type: 'integer', minimum: 0, maximum: 5 },
              contentType: { type: 'string', enum: ['text/html', 'application/xhtml+xml'] },
              parser: { type: 'string' },
            },
          },
        },
      ...
      SecurityReport: {
        ...
        properties: {
          id: { type: 'string' },
          kind: { type: 'string', enum: ['security'] },
          ...
          findings: {
            ...
            items: {
              type: 'object',
              required: ['code', 'severity', 'evidence', 'remediation'],
              properties: {
                code: { type: 'string' },
          ...
          provenance: {
            type: 'object',
            required: ['fetchedAt', 'redirects', 'contentType'],
            properties: {
              fetchedAt: { type: 'string', format: 'date-time' },
              redirects: { type: 'integer', minimum: 0, maximum: 5 },
              contentType: { type: 'string', enum: ['text/html', 'application/xhtml+xml'] },
