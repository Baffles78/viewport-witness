...
- Typecheck, lint, build, and 269 unit/integration tests passed. Local end-to-end tests passed 10 with 2 intentionally skipped. A separate live read-only exercise processed both new HTTP routes against `https://example.com`; extraction returned PASS and the security gate returned its expected evidence-based FAIL.
- Exact-source review follow-up pins each HTTPS connection to the public address that passed validation, preserves hostname/SNI/certificate checks, caps both wire and decoded bytes, and terminates redirect response bodies. Resolver-seam tests prove a simulated rebind cannot replace the pinned public address with loopback.
- Both extraction and security parsing run outside the service process in a resource-limited worker thread. The worker receives only the time remaining from the shared eight-second fetch-and-parse budget; timeout, abort, and resource failure terminate it and fail the bounded job safely. Adversarial nested-markup tests cover both parsers.
- OpenAPI status responses now include distinct ExtractReport and SecurityReport schemas. Schema tests cover actual completed report shapes and keep QAReport limited to check, verify, and compare.
